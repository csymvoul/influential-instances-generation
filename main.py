...
if model.get_type() == ModelType.BinaryClassification or model.get_type() == ModelType.MulticlassClassification:
    ...
    if found_ii:
        ...
        accuracy_list = model.get_instance_accuracy_list()
        precision_list = model.get_instance_precision_list()
        recall_list = model.get_instance_recall_list()
        f1_score_list = model.get_instance_f1_score_list()
        print("\n\n")
        print("Accuracy: \t\t{0}".format(accuracy_list))
        print("Precision: \t\t{0}".format(precision_list))
        print("Recall: \t\t{0}".format(recall_list))
        print("F1 score: \t\t{0}".format(f1_score_list))
    ...
    first_results.append(model.get_mae(forInstance=False))
    first_results.append(model.get_data().get_X_train().shape[0])
    first_results.append(end_time - start_time)
    found_ii = model.train_for_influential_instances()
    if found_ii:
        ...
        # Prediction and evaluation
        model.predict(model.get_data().get_X_test())
        final_results.append(model.get_mse(forInstance=False))
        final_results.append(model.get_rmse(forInstance=False))
        final_results.append(model.get_mae(forInstance=False))
        final_results.append(model.get_data().get_X_train().shape[0])
        final_results.append(end_time - start_time)
        print("\n\n\t\t\tInitial results: \t\t Final results:")
        print("MSE: \t\t\t{0}\t\t\t\t {1}".format(round(first_results[0], 3), round(final_results[0], 3)))
        print("RMSE: \t\t\t{0}\t\t\t\t {1}".format(round(first_results[1], 3), round(final_results[1], 3)))
        print("MAE: \t\t\t{0}\t\t\t\t {1}".format(round(first_results[2], 3), round(final_results[2], 3)))
